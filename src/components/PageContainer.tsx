...
export function PageContainer({ children, mainText, secondaryText, ...props }: PageContainerProps) {
  ...
  return (
    <Box id="pageContainer" display="flex" flexDirection={'column'} flexGrow={1} sx={{ h: '100%', flexGrow: 1, p: isMobile ? 2 : 6 }}>
      <Typography ml={2} mt={2} variant="h1" fontSize="28px" fontWeight="normal" sx={{ color: palette.text.primary }}>
        {mainText}
      </Typography>
      <Typography ml={2} sx={{ fontSize: '16px', color: palette.text.secondary, fontWeight: 'light' }}>
        {secondaryText}
      </Typography>
      {children}
    </Box>
